...
public class SaleController {
    ...
    @GetMapping(path = "/superiorstore/{num}")
    public ResponseEntity<ResponseDTO> getSalesByNum (@PathVariable int num) {
        if (saleService.getSalesByNum(num).isEmpty()) {
            return new ResponseEntity<>(new ResponseDTO(404, "No hay tiendas para cumplir este requisito", null)
                    , HttpStatus.BAD_REQUEST);
        }
        else {
            return new ResponseEntity<>(new ResponseDTO(200,saleService.getSalesByNum(num),null),HttpStatus.OK);
        }
    }
